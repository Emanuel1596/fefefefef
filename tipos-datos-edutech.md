...
Las reglas de integridad adicionales se explican al final para no saturar el diagrama.
...
| Regla | Explicacion |
|---|---|
| Examen.id_curso unico | Cada curso tiene un solo examen final. |
| Orden_Detalle con id_orden e id_curso unico | Evita repetir el mismo curso en una misma orden. |
| Modulo con id_curso y numero_orden unico | Evita dos modulos con el mismo orden dentro del curso. |
| Leccion con id_modulo y numero_orden unico | Evita dos lecciones con el mismo orden dentro del modulo. |
| Leccion_Recurso con id_leccion y numero_orden unico | Evita dos recursos en la misma posicion dentro de una leccion. |
| Intento_Examen con id_inscripcion y numero_intento unico | Controla el numero de intentos por inscripcion. |
| Intento_Pregunta con id_intento e id_pregunta unico | Evita repetir una pregunta en el mismo intento. |
| Intento_Pregunta con id_intento y numero_orden unico | Evita dos preguntas en la misma posicion del intento. |
| Pago con id_proveedor_pago e id_pago_externo unico | Evita duplicar pagos externos de PayPal o Stripe. |
| Webhook_Pago con id_evento_externo unico | Evita procesar dos veces el mismo webhook. |
